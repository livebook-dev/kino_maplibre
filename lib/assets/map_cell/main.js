...
export function init(ctx, payload) {
  ...
  const app = Vue.createApp({
        ...
        <div class="container">
          ...
          <div class="sources">
            <Accordion
              ...
              @remove-layer="removeSource(index)"
              :showDelete="hasSources"
            >
                <template v-slot:title>
                  <span>
                    Source: <span class="highlight">{{ source.source_id }}</span>
                  </span>
                </template>
                ...
                <div class="row">
                  ...
                  <BaseSelect
                    v-if="source.source_type === 'variable'"
                    name="source_data"
                    label="Source data (Geo structs)"
                    :index="index"
                    v-model="source.source_data"
          ...
          <div class="add-layer">
            <button class="button" type="button" :disabled="noSources" @click="addSource()">Add source</button>
    ...
    data() {
      return {
        ...
        typeOptions: ["fill", "line", "circle", "heatmap"],
        ...
        sourceVariables: payload.source_variables,
      };
    },
    ...
    computed: {
      ...
      sourceOptions() {
        return this.sources.map((source) => source.source_id).filter((source) => source);
      },
      hasSourceOptions() {
        return this.sourceOptions.length > 0;
      },
      hasSourceVariables() {
    ...
    methods: {
      ...
      handleFieldChange(event) {
        ...
        ctx.pushEvent("update_field", { field: name, value, idx: idx && parseInt(idx) });
      },
      addSource() {
        ctx.pushEvent("add_source");
      },
      addLayer() {
        ctx.pushEvent("add_layer");
      },
      removeSource(idx) {
        ctx.pushEvent("remove_source", { source: idx });
      },
      removeLayer(idx) {
  ...
  ctx.handleEvent("update_source", ({ idx, fields }) => {
    setSourceValues(idx, fields);
  });
  ...
  ctx.handleEvent("set_sources", ({ sources }) => {
    app.sources = sources;
  });
  ...
  ctx.handleEvent("set_source_variables", ({ source_variables }) => {
    app.sourceVariables = source_variables;
  });
  ...
  function setSourceValues(idx, fields) {
    for (const field in fields) {
      app.sources[idx][field] = fields[field];
    }
  }
